...
import { VisualizerProvider, useVisualizerContext } from './context/VisualizerContext';
import { SortingAIAssistant } from './components/SortingAIAssistant';
import { AIAssistantLogo } from './components/AIAssistantLogo';

function AppContent() {
  const [showScrollTop, setShowScrollTop] = useState(false);
  const { toggleAssistant, isAssistantOpen } = useVisualizerContext();
    ...
    <div className="min-h-screen w-full bg-slate-50/90 text-slate-900 flex flex-col font-sans selection:bg-indigo-600 selection:text-white min-w-0">
      <header className="fixed inset-x-0 top-0 z-40 border-b border-slate-200/80 bg-white flex-shrink-0">
        <div className="mx-auto w-full max-w-[1800px] px-3 sm:px-4 lg:px-6 h-14 sm:h-16 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* AI Assistant Nav Trigger - Anchored at the top of the web app */}
            <button
              id="top-ai-assistant-btn"
              type="button"
              onClick={toggleAssistant}
              className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl text-xs font-bold transition-all border cursor-pointer ${
                isAssistantOpen
                  ? 'bg-indigo-600 text-white border-indigo-600 shadow-md ring-2 ring-indigo-300/60'
                  : 'bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border-indigo-200/90 shadow-2xs hover:shadow-xs'
              }`}
              title="Open Sorting AI Assistant"
            >
              <AIAssistantLogo className={`w-4 h-4 ${isAssistantOpen ? 'text-white' : 'text-indigo-600'}`} />
              <span>Sorting AI</span>
              <span className={`w-2 h-2 rounded-full ${isAssistantOpen ? 'bg-white' : 'bg-emerald-500 animate-pulse'}`} />
            </button>
          </div>
        </div>
      </header>
      ...
      <footer className="border-t border-slate-200/80 bg-white py-3 sm:py-4 text-[10px] sm:text-xs text-slate-500 flex-shrink-0">
        <div className="mx-auto max-w-[1800px] w-full px-3 sm:px-4 lg:px-6 flex flex-col md:flex-row items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 text-slate-500">
            ...
            <span>•</span>
            <span className="text-indigo-600 font-medium">Sorting AI Assistant</span>
          </div>
        </div>
      </footer>
      ...
      {showScrollTop && (
        <button
          ...
          title="Scroll to top"
          className="fixed bottom-20 right-4 z-40 inline-flex h-10 w-10 items-center justify-center rounded-full bg-slate-900/90 text-white shadow-lg ring-1 ring-white/20 transition hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:bottom-22 sm:right-7"
        >
          <ArrowUp className="h-4 w-4" />
        </button>
      )}

      {/* Sorting AI Assistant Drawer/Modal */}
      <SortingAIAssistant />
    </div>
  );
}
